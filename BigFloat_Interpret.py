...
class Input(Interpretator):
    ...
    def interpret(self, text: str):
        ...
        for coef in coefficients:
            ...
            result_coefs.append(result.value)
        return Result.good(result_coefs, '')
...
def read_digits_with_underscores(text):
    if text == '' or not is_digit(text[0]):
        return None
    value = text[0]
    remaining = text[1:]
    while remaining:
        if is_digit(remaining[0]):
            value += remaining[0]
            remaining = remaining[1:]

        elif remaining[0] == '_':
            if len(remaining) < 2 or not is_digit(remaining[1]):
                return None 
...
class FloatParser(Interpretator):
    ...
    def interpret(self, text: str):
        ...
        if text != '':
            return Result.fail(f"лишние символы в конце: {text}", text)
...
class IntegerParser(Interpretator):
    ...
    def interpret(self, text: str):
        ...
        if not is_nonzero_digit(text[0]):
            return Result.fail('в целочисленной части ожидалась цифра', text)
        parsed = read_digits_with_underscores(text)
        if parsed is None:
            return Result.fail('некорректное использование "_" в числе', text)
        return Result.good(parsed[0], parsed[1])
...
class FractionalParser(Interpretator):
    ...
    def interpret(self, text: str):
        ...
        remaining = text[1:]
        if remaining == '' or not is_digit(remaining[0]):
            return Result.fail('после точки обязательно должно идти число', '')
        
        parsed = read_digits_with_underscores(remaining)

        if parsed is None:
            return Result.fail('некорректное использование "_" в дробной части', text)
...
class ExponentParser(Interpretator):
    ...
    def interpret(self, text: str):
        ...
        sign_value, remaining = sign_result.value, sign_result.remaining
        ...
        if parsed is None:
            return Result.fail('некорректное использование "_" в экспоненте', text)
        return Result.good(sign_value + parsed[0], parsed[1])
